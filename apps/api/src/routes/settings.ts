...
export async function registerSettingsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { thresholdId: string }; Querystring: { amber?: string; red?: string } }>(
    ...
    async (request, reply) => {
      ...
      const amber = request.query.amber === undefined ? policy.amber : Number(request.query.amber);
      const red = request.query.red === undefined ? policy.red : Number(request.query.red);
