  ...
  return (
    ...
    >
      {[0.25, 0.5, 0.75, 1].map((f) => (
        <circle key={f} cx={CENTRE} cy={CENTRE} r={MAX_RADIUS * f} fill="none" stroke="#05061f" strokeOpacity={0.08} />
      ))}
      {Array.from({ length: 12 }, (_, i) => i * 30).map((deg) => {
        const edge = point(scaleMax, deg);
        return <line key={deg} x1={CENTRE} y1={CENTRE} x2={edge.x} y2={edge.y} stroke="#05061f" strokeOpacity={0.06} />;
      })}

      <circle cx={CENTRE} cy={CENTRE} r={trimRing} fill="none" stroke="#f08c00" strokeWidth={1.2} strokeDasharray="5 4" />
      <text x={CENTRE + 4} y={CENTRE - trimRing - 5} fontSize={10} fill="#f08c00" fontWeight={600}>
      ...
      {["0°", "90°", "180°", "270°"].map((label, i) => {
        ...
        return (
          <text key={label} x={edge.x} y={edge.y + 3} fontSize={10} textAnchor="middle" fill="#4b4f77">
            {label}
          </text>
        );
      })}
      ...
      {history.map((h, index) => (
        <g key={h.shot.id}>
          <circle cx={h.x} cy={h.y} r={4} fill="#ffffff" stroke="#3b32c2" strokeOpacity={0.3 + (index / history.length) * 0.7} strokeWidth={1.6} />
        </g>
      ))}
      ...
      <circle cx={predicted.x} cy={predicted.y} r={4.5} fill="none" stroke="#0a8754" strokeWidth={1.8} strokeDasharray="3 2" />
      <circle cx={CENTRE} cy={CENTRE} r={2.5} fill="#05061f" />
    </svg>
  );
}
